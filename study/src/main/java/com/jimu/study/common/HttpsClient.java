...
public class HttpsClient {
    ...
    public static String httpsRequestReturnString(String requestUrl, String requestMethod, String postData) throws Exception{
        ...
        try{
            ...
            //设置编码
            httpsUrlConnection.setRequestProperty("Charsert", WechatConstants.DEFAULT_CHARSET);

            //判断是否需要提交数据
            if(StringUtils.equals(requestMethod,HttpsClient.METHOD_POST) && StringUtils.isNotBlank(postData)){
                //讲参数转换为字节提交
                byte[] bytes = postData.getBytes(WechatConstants.DEFAULT_CHARSET);
                //设置头信息
                ...
                outputStream.write(postData.getBytes(WechatConstants.DEFAULT_CHARSET));
                outputStream.flush();
                outputStream.close();
            }else{
                //开始连接
